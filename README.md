...
3. Download the pipeline and test it on a minimal dataset with a single command:
   ...
   ```bash
   nextflow run dhslab/wgsnano -profile test,YOURPROFILE(S) --outdir <OUTDIR>
   ```
...
4. Start running your own analysis!
   ...
   ```bash
   nextflow run dhslab/wgsnano --input samplesheet.csv --fasta <FASTA> -profile <docker/singularity/podman/shifter/charliecloud/conda/institute> --outdir <OUTDIR> 
   ```
...
```bash
git clone https://github.com/dhslab/wgsnano.git
cd wgsnano/
